...
def build_contact_items(contact: dict) -> list[str]:
    ...
    if contact.get("email"):
        items.append(f'<span class="contact-item"><a href="mailto:{contact["email"]}" target="_blank" rel="noopener noreferrer">Email</a></span>')
    if contact.get("personal_website"):
        items.append(f'<span class="contact-item"><a href="{contact["personal_website"]}" target="_blank" rel="noopener noreferrer">Website</a></span>')
    if contact.get("linkedin"):
        items.append(f'<span class="contact-item"><a href="{contact["linkedin"]}" target="_blank" rel="noopener noreferrer">LinkedIn</a></span>')
    if contact.get("google_scholar"):
        items.append(f'<span class="contact-item"><a href="{contact["google_scholar"]}" target="_blank" rel="noopener noreferrer">Google Scholar</a></span>')
    return items
...
def render_authors_with_tags(authors: str, name: str) -> dict:
    ...
    for i, part in enumerate(parts):
        ...
        if stripped.startswith("[FA]"):
            ...
            if stripped == name:
                result.append(f'<span class="underline-self">{escape(stripped)}</span>{SYMBOL_FA}')
            else:
            ...
            stripped = stripped[4:].strip()
            if stripped == name:
                result.append(f'<span class="underline-self">{escape(stripped)}</span>{SYMBOL_CF}')
            else:
...
def build_sections(data: dict, note_map: dict = None) -> list[dict]:
    ...
    if section_exists(data, "research"):
        ...
        if talk_map:
            ...
            for parent_title, talks in talk_map.items():
                author_result = render_authors_with_tags(talks[0]["parent_authors"], user_name)
                parent_authors_html = re.sub(r'<sup class="author-symbol">[^<]*</sup>', '', author_result["authors_html"])
                talk_entries = []
                for t in talks:
                    month = str(t.get("month", "")).strip()
                    year_str = str(t.get("year", "")).strip()
                    date = f"{month} {year_str}".strip() if month and year_str else year_str
        ...
        if pres_map:
            sections.append({"type": "heading", "title": "Conference & Workshop Presentations"})
            ...
            for idx, num in enumerate(sorted(note_map.keys(), key=int)):
                ...
                legend_parts.append(f'<sup class="pres-note">{symbol}</sup> {escape(note_map[num])}')
            legend_html = " | ".join(legend_parts) if legend_parts else ""
            sections.append({"type": "conference_legend", "data": {"legend": legend_html}})
            
            for parent_title, pres_list in pres_map.items():
                author_result = render_authors_with_tags(pres_list[0]["parent_authors"], user_name)
                parent_authors_html = re.sub(r'<sup class="author-symbol">[^<]*</sup>', '', author_result["authors_html"])
                pres_entries = []
                for p in pres_list:
                    ...
                    if notes:
                        ...
                        for idx, num in enumerate(sorted(note_map.keys(), key=int)):
                            ...
                            if tag in notes:
                                symbol = symbols[idx] if idx < len(symbols) else num
                                mapped_notes.append(f'<sup class="pres-note">{symbol}</sup>')
                        ...
                        if bracket_match:
                            ...
                            if content and not content.isdigit():
                                ...
                                content = re.sub(r'^(b|i|u|em|strong)\s*', '', content, flags=re.IGNORECASE)
                                content = re.sub(r'\s*/\s*(b|i|u|em|strong)$', '', content, flags=re.IGNORECASE)
                                content = content.strip()
                                if content:
                                    # Re-wrap with [b] tags so format_text() can process them
                                    unmatched_notes.append(format_text(f'[b]{content}[/b]'))
                        ...
                        conference_html = "".join(mapped_notes) + conference_html if mapped_notes else conference_html
                        if remaining:
                    ...
                    pres_entries.append({
                        ...
                        "location": format_text(p.get("location", "")) if p.get("location") else "",
    ...
    if section_exists(data, "awards"):
        ...
        awards = sorted(data["awards"], key=lambda a: extract_year(str(a.get("year", ""))) or 0, reverse=True)
        award_entries = []
        for a in awards:
    ...
    # Teaching Experience
    if section_exists(data, "teaching_experience"):
        sections.append({"type": "heading", "title": "Teaching Experience"})
        teaching = sorted(data["teaching_experience"], key=lambda t: t.get("year", 0), reverse=True)
        teaching_items = []
        for t in teaching:
...
def render_cv(data: dict, style: dict) -> str:
    ...
    context["name"] = personal_section["data"]["full_name"] if personal_section else "CV"
